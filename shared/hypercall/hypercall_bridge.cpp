...
std::uint64_t HyperCall::ReadGuestVirtualMemory(void* const guest_destination_buffer,
...
std::uint64_t HyperCall::WriteGuestVirtualMemory(const void* const guest_source_buffer,
...
std::uint64_t HyperCall::ReadGuestCr3()
{
    return MakeHypercall(hypercall_type_t::_hc_0x140, 0, 0, 0, 0);
}

std::uint64_t HyperCall::InjectMouseMovement(long x, long y)
{
...
bool HyperCall::SetMouseHookAddress(std::uint64_t ept_hook_address)
{
