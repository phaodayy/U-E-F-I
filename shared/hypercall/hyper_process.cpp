...
namespace
{
    ...
    bool ReadKernelValue(const std::uint64_t address, value_type* value)
    {
        ...
        return HyperCall::ReadGuestVirtualMemory(value, address, g_CurrentCr3, sizeof(value_type)) == sizeof(value_type);
    }
    ...
    bool ResolvePsInitialSystemProcess()
    {
        ...
        g_CurrentCr3 = HyperCall::ReadGuestCr3();
        if (g_CurrentCr3 == 0)
        {
            return false;
        }
...
bool HyperProcess::Initialize()
{
    return ResolvePsInitialSystemProcess();
}

bool HyperProcess::QueryProcessData(const std::uint32_t pid, query_process_data_packet* const output, const char* target_name)
{
    if (output == nullptr)
    {
        return false;
    ...
    if (HyperCall::ReadGuestVirtualMemory(&current_eprocess, g_PsInitialSystemProcess, walk_cr3, 8) != 8 || current_eprocess == 0)
    {
        return false;
    }
    ...
    do
    {
        ...
        HyperCall::ReadGuestVirtualMemory(&current_pid, current_eprocess + eprocess::UniqueProcessId, walk_cr3, 8);
        HyperCall::ReadGuestVirtualMemory(&current_cr3, current_eprocess + eprocess::DirectoryTableBase, walk_cr3, 8);
        HyperCall::ReadGuestVirtualMemory(&current_base, current_eprocess + eprocess::SectionBaseAddress, walk_cr3, 8);
        HyperCall::ReadGuestVirtualMemory(&current_peb, current_eprocess + eprocess::Peb, walk_cr3, 8);
        
        std::uint64_t current_threads = 0;
        HyperCall::ReadGuestVirtualMemory(&current_threads, current_eprocess + 0x5f0, walk_cr3, 4); 
        ...
        if (HyperCall::ReadGuestVirtualMemory(name, current_eprocess + g_ImageFileNameOffset, walk_cr3, 15) > 0) {
            ...
            if (is_match) {
                if (current_cr3 != 0 && current_base != 0 && current_peb != 0) {
                    if (pid != 0 || target_name != nullptr) {
...
LONG_PTR HyperProcess::GetWindowStyleStealthily(HWND hwnd, uint32_t pid)
{
    return GetWindowLongPtr(hwnd, GWL_EXSTYLE);
}

bool HyperProcess::SetWindowStyleStealthily(HWND hwnd, LONG_PTR new_style)
{
    return false; 
}

std::vector<uint32_t> HyperProcess::GetSafeOverlayPids()
{
...
std::vector<uint32_t> HyperProcess::FindAllPidsByGhostWalk(const char* target_name)
{
    std::vector<uint32_t> found_pids;
    if (!ResolvePsInitialSystemProcess()) return found_pids;

    std::uint64_t current_eprocess = 0;
    if (HyperCall::ReadGuestVirtualMemory(&current_eprocess, g_PsInitialSystemProcess, g_CurrentCr3, 8) != 8 || current_eprocess == 0) {
        return found_pids;
    }
    ...
    do {
        std::uint64_t pid = 0;
        HyperCall::ReadGuestVirtualMemory(&pid, current_eprocess + eprocess::UniqueProcessId, g_CurrentCr3, 8);

        char name[16] = {};
        if (HyperCall::ReadGuestVirtualMemory(name, current_eprocess + g_ImageFileNameOffset, g_CurrentCr3, 15) > 0) {
        ...
        std::uint64_t next_link = 0;
        HyperCall::ReadGuestVirtualMemory(&next_link, current_eprocess + eprocess::ActiveProcessLinks, g_CurrentCr3, 8);
        current_eprocess = next_link - eprocess::ActiveProcessLinks;
        check_count++;
...
std::vector<kernel_window_data> HyperProcess::EnumerateWindowsStealthily()
{
...
DWORD HyperProcess::FindProcessIdByName(const wchar_t* const process_name)
{
