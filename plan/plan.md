...
*   **Bước 3.1:** Tích hợp bộ thư viện `XorStr.h` (từ `Game-2\PubgExt\Security`) vào Node. Đây là "vũ khí" quan trọng giúp mã hóa chuỗi tại thời điểm biên dịch (Compile-time).
*   **Bước 3.2:** Thay thế toàn bộ các chuỗi nhạy cảm bằng Macro `_XOR_`. Các chuỗi cần ưu tiên:
    *   Tên tiến trình mục tiêu (`TslGame.exe`).
    *   Tên lớp cửa sổ (`Chrome_WidgetWin_1`, `Discord Overlay`).
    *   Toàn bộ các chuỗi Debug và Log (`[+] Found...`, `[-] Error...`).
    *   Tên các lớp và biến nội bộ có nguy cơ bị Signature.
*   **Mục tiêu:** Khi kẻ thù quét bộ nhớ, họ sẽ không thấy bất kỳ văn bản (PlainText) nào liên quan đến logic của chúng ta.
