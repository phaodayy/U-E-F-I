...
Vector3 GetBoneWorldPos(int boneIndex, uint64_t boneArray, uint64_t meshPtr) {
  DFTransform compToWorld =
      Read<DFTransform>(meshPtr + VDataOffsets::ComponentToWorld());
...
bool Initialize(uint32_t process_id, uint64_t base_address) {
  ...
  uint64_t uworldPtr1 =
      Read<uint64_t>(g_BaseAddress + VDataOffsets::GWorld());
  G_UWorld = Read<uint64_t>(uworldPtr1);

  if (G_UWorld && G_UWorld > 0x10000) {
    G_GameInstance = Read<uint64_t>(G_UWorld + VDataOffsets::GameInstance());
    G_PersistentLevel =
        Read<uint64_t>(G_UWorld + VDataOffsets::PersistentLevel());
...
void UpdateGameData() {
  ...
  uint64_t uworldPtr1 = Read<uint64_t>(g_BaseAddress + VDataOffsets::GWorld());
  ...
  */
  ...
  if (!G_LocalPlayer || !G_PlayerController || !G_LocalPawn) {
    uint64_t localPlayersArray =
        Read<uint64_t>(G_GameInstance + VDataOffsets::LocalPlayers());
    if (localPlayersArray) {
      G_LocalPlayer = Read<uint64_t>(localPlayersArray);
      if (G_LocalPlayer) {
        G_PlayerController =
            Read<uint64_t>(G_LocalPlayer + VDataOffsets::PlayerController());
        if (G_PlayerController) {
          G_LocalPawn = Read<uint64_t>(G_PlayerController +
                                       VDataOffsets::AcknowledgedPawn());
        }
      }
    }
    /*
    ...
    */
    ...
    uint64_t checkPawn =
        Read<uint64_t>(G_PlayerController + VDataOffsets::AcknowledgedPawn());
  ...
  if (G_PlayerController) {
    uint64_t camMgr = Read<uint64_t>(G_PlayerController +
                                     VDataOffsets::PlayerCameraManager());
    if (camMgr) {
      struct DV3 {
        double x, y, z;
      };
      ...
      FMinViewInfo vi = Read<FMinViewInfo>(
          camMgr + VDataOffsets::CameraCachePrivate() + 0x10);
  ...
  uint64_t localMesh = Read<uint64_t>(G_LocalPawn + VDataOffsets::Mesh());
  Vector3 localPos = {0, 0, 0};
  if (localMesh) {
    uint64_t localBoneArr =
        Read<uint64_t>(localMesh + VDataOffsets::BoneArray());
    if (!localBoneArr)
      localBoneArr =
          Read<uint64_t>(localMesh + VDataOffsets::BoneArrayCache());
    if (localBoneArr)
  ...
  uint64_t actorArray =
      Read<uint64_t>(G_PersistentLevel + VDataOffsets::ActorArray());
  int actorCount = Read<int>(G_PersistentLevel + VDataOffsets::ActorCount());
  ...
  */
  ...
  for (int i = 0; i < actorCount; i++) {
    ...
    uint64_t mesh = Read<uint64_t>(actor + VDataOffsets::Mesh());
    if (!mesh) {
      actorSkips_mesh++;
      continue;
    }
    ...
    uint64_t damageHandler =
        Read<uint64_t>(actor + VDataOffsets::DamageHandler());
    if (!damageHandler) {
      actorSkips_damage++;
      continue;
    }
    ...
    if (GUI::v_friend) {
      bool wasAlly = Read<bool>(actor + VDataOffsets::WasAlly());
      if (wasAlly) {
        actorSkips_team++;
        continue;
      }
    ...
    float health = Read<float>(damageHandler + VDataOffsets::Health());
    if (health <= 0.0f || health > 200.0f) {
      actorSkips_health++;
      continue;
    }

    uint64_t boneArray = Read<uint64_t>(mesh + VDataOffsets::BoneArray());
    if (!boneArray)
      boneArray = Read<uint64_t>(mesh + VDataOffsets::BoneArrayCache());
    if (!boneArray) {
      actorSkips_bones++;
      continue;
    }
    ...
    int boneCount = Read<int>(mesh + VDataOffsets::BoneCount());
    ...
    player.bIsAlive = Read<bool>(damageHandler + VDataOffsets::bAlive());
    player.bIsBot = (player.Name == "Bot");

    float lastSubmit = Read<float>(mesh + VDataOffsets::LastSubmitTime());
    float lastRender =
        Read<float>(mesh + VDataOffsets::LastRenderTime());
    ...
    uint64_t inventory = Read<uint64_t>(actor + VDataOffsets::Inventory());
    if (inventory) {
      uint64_t equippable =
          Read<uint64_t>(inventory + VDataOffsets::CurrentEquippable());
      if (equippable) {
