        ...
        if (status == EFI_SUCCESS)
        {
            ...
            else
            {
                ...
                if (status == EFI_SUCCESS)
                {
                    ...
                    if (dos_header->e_magic != 0x5a4d || dos_header->e_lfanew + sizeof(EFI_IMAGE_NT_HEADERS64) > hyperv_attachment_file_size)
                    {
                    ...
                    {
                        EFI_IMAGE_NT_HEADERS64* nt_headers = image_get_nt_headers(*file_buffer_out);
