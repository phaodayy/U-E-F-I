...
namespace Utils
{
	...
	inline bool IsLobby(const std::string& mapName) {
		...
		return true; // Mac dinh la Lobby neu khong khop cac map vao tran
	}
	...
	{
		return (bool)(Ptr >= 0x1000000 && Ptr < 0x7FFFFFF00000 && (Ptr % sizeof(uint64_t)) == 0);
	}
	...
	inline bool ValidPtr(PVOID Ptr)
	{
		...
		return (bool)(v1 >= 0x1000000 && v1 < 0x7FFFFFF00000 && (v1 % sizeof(uint64_t)) == 0);
	}
	...
	inline std::string UnicodeToAnsi(const wchar_t* wstr)
	{
		if (!wstr) return "";
	...
	// 处理原有的 char* 输入（保持向后兼容）
	inline std::string UnicodeToAnsi(char* wstr)
	{
		if (!wstr) return "";
		return UnicodeToAnsi(reinterpret_cast<const wchar_t*>(wstr));
	}
