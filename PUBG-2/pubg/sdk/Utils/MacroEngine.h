...
public:
    ...
    static void UpdateAttachments(const uint64_t weapon)
    {
        ...
        const uint64_t attachedBase = PubgMemory::Read<uint64_t>(weapon + pubg_config::offsets::AttachedItems);
        const int attachedCount = PubgMemory::Read<int>(weapon + pubg_config::offsets::AttachedItems + 0x8);
        if (attachedBase > 0x10000 && attachedCount > 0 && attachedCount <= 10)
        {
            for (int i = 0; i < attachedCount; ++i)
            {
                const uint64_t attachItem = PubgMemory::Read<uint64_t>(attachedBase + (i * 8));
                if (attachItem < 0x10000) continue;

                uint64_t itemTableRow = PubgMemory::Read<uint64_t>(attachItem + pubg_config::offsets::WeaponAttachmentData);
                if (itemTableRow < 0x10000)
                {
                    itemTableRow = PubgMemory::Read<uint64_t>(attachItem + pubg_config::offsets::ItemTable);
                }
                if (itemTableRow < 0x10000) continue;

                const uint32_t itemID = PubgMemory::Read<uint32_t>(itemTableRow + pubg_config::offsets::ItemID);
                if (!itemID) continue;
    ...
    static void ForceScan()
    {
        ...
        const uint64_t weaponProc = PubgMemory::Read<uint64_t>(G_LocalPawn + pubg_config::offsets::WeaponProcessor);
        if (!weaponProc) return;

        const uint8_t currentIdx = PubgMemory::Read<uint8_t>(weaponProc + pubg_config::offsets::CurrentWeaponIndex);
        if (currentIdx >= 3)
        {
        ...
        const uint64_t equippedAddr = PubgMemory::Read<uint64_t>(weaponProc + pubg_config::offsets::EquippedWeapons);
        const uint64_t weapon = PubgMemory::Read<uint64_t>(equippedAddr + (currentIdx * 8));
        if (!weapon)
        {
            current_weapon_name = "None";
            current_gun_data = json();
        ...
        const uint32_t objID = pubg_config::decrypt_cindex(PubgMemory::Read<uint32_t>(weapon + pubg_config::offsets::ObjID));
    ...
    static void Update()
    {
        ...
        const uint64_t mesh = PubgMemory::Read<uint64_t>(G_LocalPawn + pubg_config::offsets::Mesh);
        const uint64_t anim = mesh ? PubgMemory::Read<uint64_t>(mesh + pubg_config::offsets::AnimScriptInstance) : 0;

        // --- PAOD Context Acquisition ---
        float currentFOV = 80.0f;
        const uint64_t cameraManager = PubgMemory::Read<uint64_t>(G_PlayerController + pubg_config::offsets::PlayerCameraManager);
        if (cameraManager) {
            currentFOV = PubgMemory::Read<float>(cameraManager + pubg_config::offsets::CameraCacheFOV);
        }
        if (currentFOV < 10.0f || currentFOV > 170.0f) currentFOV = 80.0f;

        bool isInVehicle = false;
        const uint64_t riderComp = PubgMemory::Read<uint64_t>(G_LocalPawn + pubg_config::offsets::VehicleRiderComponent);
        if (riderComp) {
            int seatIdx = PubgMemory::Read<int>(riderComp + pubg_config::offsets::SeatIndex);
            if (seatIdx != -1) isInVehicle = true;
        }

        bool isReloading = anim ? PubgMemory::Read<bool>(anim + pubg_config::offsets::bIsReloading_CP) : false;
        uint8_t characterState = PubgMemory::Read<uint8_t>(G_LocalPawn + pubg_config::offsets::CharacterState);
        ...
        if (fireHeld)
        {
            ...
            const AngleRotation recoilRot = PubgMemory::Read<AngleRotation>(anim + pubg_config::offsets::RecoilADSRotation_CP);
        ...
        bool isTargetVisible = GameData.AimBot.TargetPlayerInfo.IsVisible;

        if (triggerActive && GameData.AimBot.Target != 0 && isScoping && !isReloading && !isSwimming && isTargetVisible)
        {
                auto skeleton = GameData.AimBot.TargetPlayerInfo.Skeleton;
                if (skeleton != nullptr)
                {
