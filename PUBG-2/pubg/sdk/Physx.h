...
namespace GamePhysX
{
	...
	__forceinline std::vector<TriangleMeshData> GetMeshData(
		...
		size_t batchSize = 40; // Giam tai cho DMA Handle de khong bi treo toan bo ESP
		for (size_t startIdx = 0; startIdx < objects.size(); startIdx += batchSize) {
			...
			// Ngu 5ms sau moi lô nho 40 object, giup Thread DrawESP va luong AimBot doc duoc Memory khong bi chan PCI-E
