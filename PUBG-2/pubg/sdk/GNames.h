...
{
public:
    static void ReadGNamesImpl(std::vector<int> ids)
    {
        ...
        {
            mem.AddScatterRead(hScatter, GameData.GNames + ((int(gNameInfo.ID / Offset::ChunkSize)) * 8), (uint64_t*)&gNameInfo.pGName);
        }
        mem.ExecuteReadScatter(hScatter);

        for (GNameInfo& gNameInfo : gNameInfos)
        {
            mem.AddScatterRead(hScatter, gNameInfo.pGName + ((int(gNameInfo.ID % Offset::ChunkSize)) * 8), (uint64_t*)&gNameInfo.pName);
        }
        mem.ExecuteReadScatter(hScatter);
        ...
        std::unordered_map<std::string, EntityInfo, FnvHash> NewGNameLists;
        std::unordered_map<int, EntityInfo> NewGNameListsByID;
        ...
        for (GNameInfo& gNameInfo : gNameInfos)
        {
            if (gNameInfo.ID > 0) {
                ...
                if (GNameLists.count(name) > 0) {
                    ...
                    NewGNameLists[name] = GNameLists[name];
                    NewGNameListsByID[gNameInfo.ID] = GNameLists[name];
                }
                else {
                    ...
                    NewGNameLists[name] = entityInfo;
                    NewGNameListsByID[gNameInfo.ID] = entityInfo;
                }
            }
        }

        Data::SetGNameListsAndGNameListsByID(NewGNameLists, NewGNameListsByID);
        mem.CloseScatterHandle(hScatter);
    }

    static void ReadGNames(std::vector<int> ids)
    {
        size_t batchSize = 100; // Chia nho batch doc GNames de khong qua tai DMA
        for (size_t startIdx = 0; startIdx < ids.size(); startIdx += batchSize) {
            size_t endIdx = (std::min)(startIdx + batchSize, ids.size());
            std::vector<int> batchIds(ids.begin() + startIdx, ids.begin() + endIdx);
            ReadGNamesImpl(batchIds);
            std::this_thread::sleep_for(std::chrono::milliseconds(5)); // Giai phong PCI-E 5ms cho nhanh the, tranh Deadlock
        }
    }

    static std::string GetName_Item(int ID)
    {
        UINT64 fNamePtr = mem.Read<UINT64>(GameData.GNames + ((int(ID / Offset::ChunkSize)) * 8));
        UINT64 fName = mem.Read<UINT64>(fNamePtr + ((int(ID % Offset::ChunkSize)) * 8));
        char names_c[258];
    ...
    static uint64_t GetGNamesPtr()
    {
        ...
        uint64_t gnames = 0;
        uint64_t tmp = mem.Read<uint64_t>(GameData.GameBase + Offset::GNames);
        DWORD64 v11 = Decrypt::Xe(tmp);
        v11 = mem.Read<uint64_t>(v11 + Offset::GNamesPtr);
        gnames = Decrypt::Xe(v11);
    ...
    static void GetGNameLists()
    {
        ...
        {
            GNameInfo Info;
            Info.Offset = GameData.GNames + ((int(i / Offset::ChunkSize)) * 8);
            Info.ID = i;
        ...
        for (GNameInfo& Item : GNameInfos)
        {
            mem.AddScatterRead(hScatter, Item.Entity + ((int(Item.ID % Offset::ChunkSize)) * 8), (uint64_t*)&Item.pEntity);
        }
        mem.ExecuteReadScatter(hScatter);
