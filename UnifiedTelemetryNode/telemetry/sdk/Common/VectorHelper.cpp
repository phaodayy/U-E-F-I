...
FVector VectorHelper::GetBoneWithRotation(FTransform BoneArray, FTransform ComponentToWorld)
{
	...
	if (std::max(std::max(abs(Bone.X), abs(Bone.Y)), abs(Bone.Z)) >= 5000)
	{
...
FVector VectorHelper::GetBoneWithRotation(const FTransform& BoneArray, const FMatrix& ComponentToWorldMatrix)
{
	...
	FMatrix Matrix = BoneArray.ToMatrixWithScale() * ComponentToWorldMatrix;
	return FVector(Matrix._41, Matrix._42, Matrix._43);
}

FVector2D VectorHelper::WorldToScreen(const FVector& WorldLocation, bool BaseOrigin) {
	return WorldToScreen(WorldLocation, Data::GetCamera(), BaseOrigin);
}

FVector2D VectorHelper::WorldToScreen(const FVector& WorldLocation, const CameraData& Camera, bool BaseOrigin) {
	return WorldToScreen(WorldLocation, Camera.Location, Camera.Rotation.GetMatrix(), Camera.FOV, BaseOrigin);
}

FVector2D VectorHelper::WorldToScreen(const FVector& WorldLocation, const FVector& CameraLocationInput, const FMatrix& RotationMatrix, float FOV, bool BaseOrigin) {
	...
	if (BaseOrigin) {
		CameraLocation = CameraLocationInput + GameData.Radar.WorldOriginLocation;
	}
	else {
		CameraLocation = CameraLocationInput;
	}
