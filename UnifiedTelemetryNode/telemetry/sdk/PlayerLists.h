...
namespace MenuPlayerLists
{
    ...
    inline void SortItemsByGamePlayerInfo(std::vector<GamePlayerInfo>& items, int column, bool ascending, std::unordered_map<std::string, PlayerRankList> PlayerRankLists = {}) {
        auto compare = [&column, &ascending, &PlayerRankLists](const GamePlayerInfo& a, const GamePlayerInfo& b) {
            ...
            switch (column) {
                ...
                case 8: return ascending ? a.telemetryIdData.SurvivalLevel < b.telemetryIdData.SurvivalLevel : a.telemetryIdData.SurvivalLevel > b.telemetryIdData.SurvivalLevel;
    ...
    inline void Render(const ImFontAtlas* FontAtlas)
    {
        ...
        if (GameData.Config.Window.Players && ImGui::Begin(U8("玩家列表"), &GameData.Config.Window.Players, ImGuiWindowFlags_NoBackground | ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_NoDocking | ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoResize))
        {
            ...
            ImGui::SameLine();
            ImGui::SetNextItemWidth(100);
            ImGui::InputTextEx(U8("赛季选择"), "30", GameData.Config.signal_overlay.RankSize, IM_ARRAYSIZE(GameData.Config.signal_overlay.RankSize), ImVec2(0, 0), NULL);
...
;
            ...
            if (ImGui::BeginTable(U8("Danh Sach Player"), 11, flags, ImVec2(800.f, 365.0f)))
            {
                ImGui::TableSetupScrollFreeze(0, 1);
                ImGui::TableSetupColumn(U8("  Team"), ImGuiTableColumnFlags_WidthFixed | ImGuiTableColumnFlags_DefaultSort, 50.0f);
                ImGui::TableSetupColumn(U8("Ten Player"), ImGuiTableColumnFlags_WidthFixed, 125.0f);
                ImGui::TableSetupColumn(U8("Rank"), ImGuiTableColumnFlags_WidthFixed, 70.0f);
                ImGui::TableSetupColumn(U8("Diem"), ImGuiTableColumnFlags_WidthFixed, 50.f);
                ...
                ImGui::TableSetupColumn(U8("Loai"), ImGuiTableColumnFlags_WidthFixed, 60.0f);
                ...
                for (auto& player : players)
                {
                    ...
                    if (player.StatusType == 8 || player.StatusType == 12) {
                        ...
                        if ((!filterPlayerListIndex && !filterPlayerTypeIndex && !filterPlayerName[0]) || (filterPlayerListIndex > 0 && strstr(filterPlayerList[filterPlayerListIndex], PlayerListType.c_str())) || (filterPlayerTypeIndex > 0 && strstr(PlayerType.c_str(), filterPlayerType[filterPlayerTypeIndex])) || (filterPlayerName[0] && strstr(player.PlayerName.c_str(), filterPlayerName) != nullptr)) {
                            ...
                            ImGui::TextUnformatted(std::to_string(player.telemetryIdData.SurvivalLevel).c_str());
                            ImGui::TableSetColumnIndex(9);
                            ImGui::TextUnformatted(PlayerType.c_str());
                            if (ImGui::TableSetColumnIndex(10)) {
                                auto NeedPopStyleColor = false;
                                if (player.ListType == 1) {
                                    ImGui::PushStyleColor(ImGuiCol_Button, Utils::FloatToImColor(GameData.Config.signal_overlay.Color.Blacklist.Info));
                                    ImGui::PushStyleColor(ImGuiCol_ButtonHovered, Utils::FloatToImColor(GameData.Config.signal_overlay.Color.Blacklist.Info));
                                    ImGui::PushStyleColor(ImGuiCol_ButtonActive, Utils::FloatToImColor(GameData.Config.signal_overlay.Color.Blacklist.Info));
                                    NeedPopStyleColor = true;
                                }
                                else if (player.ListType == 2) {
                                    ImGui::PushStyleColor(ImGuiCol_Button, Utils::FloatToImColor(GameData.Config.signal_overlay.Color.Whitelist.Info));
                                    ImGui::PushStyleColor(ImGuiCol_ButtonHovered, Utils::FloatToImColor(GameData.Config.signal_overlay.Color.Whitelist.Info));
                                    ImGui::PushStyleColor(ImGuiCol_ButtonActive, Utils::FloatToImColor(GameData.Config.signal_overlay.Color.Whitelist.Info));
                                    NeedPopStyleColor = true;
                                }
