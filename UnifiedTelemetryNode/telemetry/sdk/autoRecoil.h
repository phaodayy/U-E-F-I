...
public:
	...
	static void autoRecoil()
	{
		...
		while (true)
		{
			...
			if (!GameData.Config.precision_calibration.Recoilenanlek)
			{
			...
			if (GameData.Keyboard.IsKeyDown(1))
			{
				...
				if (fov >= 70) { 
					currentRecoilValue = (float)GameData.Config.precision_calibration.yRecoil[0];
					msGap = GameData.Config.precision_calibration.interval[0];
				} else if (fov >= 40) { 
					currentRecoilValue = (float)GameData.Config.precision_calibration.yRecoil[1];
					msGap = GameData.Config.precision_calibration.interval[1];
				} else if (fov >= 25) { 
					currentRecoilValue = (float)GameData.Config.precision_calibration.yRecoil[2];
					msGap = GameData.Config.precision_calibration.interval[2];
				} else if (fov >= 18) { 
					currentRecoilValue = (float)GameData.Config.precision_calibration.yRecoil[3];
					msGap = GameData.Config.precision_calibration.interval[3];
				} else if (fov >= 12) { 
					currentRecoilValue = (float)GameData.Config.precision_calibration.yRecoil[4];
					msGap = GameData.Config.precision_calibration.interval[4];
				} else { 
					currentRecoilValue = (float)GameData.Config.precision_calibration.yRecoil[5];
					msGap = GameData.Config.precision_calibration.interval[5];
				}
