...
public:
	...
	static void UpdateFogPlayers()
	{
		while (true)
		{
			...
			TArray<uint64_t> dormantCharacterClientList = mem.Read<TArray<uint64_t>>(GameData.Antiintegrity_monitorCharacterSyncManager + Offset::DormantCharacterClientList);
		...
		if (GameData.Scene == Scene::Gaming)
		{
			...
			for (size_t i = 0; i < playerLists.size(); i += batchSize) {
				...
				for (size_t j = i; j < end; ++j) {
					...
					mem.AddScatterRead(scatterHandle, player.pPlayerInfo + Offset::SurvivalTier, (int*)&player.telemetryIdData.SurvivalTier);
					mem.AddScatterRead(scatterHandle, player.pPlayerInfo + Offset::SurvivalLevel, (int*)&player.telemetryIdData.SurvivalLevel);
					hasNewPlayers = true;
				}
			...
			for (GamePlayerInfo& player : playerLists)
			{
				...
				if (player.telemetryIdData.SurvivalTier > 0) player.telemetryIdData.SurvivalLevel = (player.telemetryIdData.SurvivalTier - 1) * 500 + player.telemetryIdData.SurvivalLevel;
			}
			...
			for (GamePlayerInfo& player : playerLists)
			{
				...
				if (playerRankLists.count(player.PlayerName) == 0)
				{
					...
					playerRankList.Survivallevel = player.telemetryIdData.SurvivalLevel;
...
private:
	...
	static CameraData ReadPlayerData(VMMDLL_SCATTER_HANDLE hScatter, std::vector<Player*>& allPlayers, std::unordered_map<uint64_t, Player>& playersData, bool bSlowUpdate) {
		...
		// Kiem tra precision_calibration active de quyet dinh doc AimOffsets hay khong
		bool needAimOffsets = GameData.Config.precision_calibration.Enable;
		...
		for (size_t i = 0; i < allPlayers.size(); i += batchSize) {
			...
			for (size_t j = i; j < end; ++j) {
				...
				// Chi doc AimOffsets khi precision_calibration bat (tiet kiem ~N*8 bytes/frame khi precision_calibration tat)
				if (needAimOffsets)
	...
	static void ProcessPlayerData(std::unordered_map<uint64_t, Player>& cachePlayers, std::unordered_map<uint64_t, Player>& playersData, int& fogPlayerCount, std::vector<Player*>& playersNeedingBones, std::vector<Player*>& playersNeedingWeaponPtr, const CameraData& camera) {
		...
		for (auto& item : cachePlayers)
		{
			...
			if (!player.Name.empty() && gamePlayerLists.count(player.Name)) {
				...
				player.SurvivalLevel = playerInfo.telemetryIdData.SurvivalLevel;
			...
			player.TeamID = (player.TeamID >= 100000) ? (player.TeamID - 100000) : player.TeamID;
			if (GameData.Config.signal_overlay.duiyou) player.IsMyTeam = false;
	...
	static void FinalizePlayerUpdate(std::unordered_map<uint64_t, Player>& cachePlayers, std::unordered_map<uint64_t, Player>& playersData, std::unordered_map<uint64_t, tMapInfo>& enemyInfoMap, precision_calibrationConfig& config, bool isWheelKeyDown, const CameraData& camera) {
		for (auto& item : cachePlayers) {
			Player& player = item.second;
		...
		for (auto& item : cachePlayers)
		{
			...
			if (GameData.precision_calibration.Target == player.Entity) GameData.precision_calibration.TargetPlayerInfo = player;
		}
	}
...
public:
	static void Update()
	{
		...
		while (true)
		{
			...
			if ((!GameData.precision_calibration.Lock || Utils::ValidPtr(GameData.precision_calibration.Target)) && isWheelKeyDown && config.AimWheel)
			{
				...
				for (auto& item : vehicleWheels) {
					...
					if (dist <= config.WheelFOV && dist < GameData.precision_calibration.ScreenDistance) {
						GameData.precision_calibration.ScreenDistance = dist; GameData.precision_calibration.Target = wheel.Wheel; GameData.precision_calibration.Bone = 0; GameData.precision_calibration.Type = EntityType::Wheel;
					}
				}
			}
