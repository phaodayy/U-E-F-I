...
namespace GamePhysX
{
	...
	std::vector<T> ReadVec(uintptr_t address, size_t size, size_t custom_type_size = NULL)
	{
		...
		size_t allocation_size = custom_type_size ? custom_type_size : sizeof(T) * size;
		if (allocation_size > 40000000) // 40MB limit for safety
			return temp;

		temp.resize(custom_type_size ? custom_type_size / sizeof(T) : size);

		if (mem.Read(address, temp.data(), allocation_size))
			return temp;
	...
	template <class _Pr>
	__forceinline std::vector<TriangleMeshData> GetMeshDataImpl(
...

		{
			if (shapeData.mType == PxGeometryType::eCONVEXMESH)
			{
				...
				for (uint32_t i = 0; i < nbPolygons; i++) {
					mem.AddScatterRead<HullPolygonDataT>(hScatter, mPolygons + (i * size), &shapeData.polygons[i]);
				}
			}
		}
		...
		for (auto& ShapeData : ShapeDatas)
		{
			...
			else if (ShapeData.mType == PxGeometryType::eTRIANGLEMESH)
			{
				...
				mem.AddScatterRead<TriangleMeshT>(hScatter, (uintptr_t)triangle_geometry.mTriangleMesh, &ShapeData.TriangleMesh);
			}
		}
		mem.ExecuteReadScatter(hScatter);
	...
	__forceinline std::vector<TriangleMeshData> GetMeshData(
		...
		std::vector<TriangleMeshData> AllMeshDatas;
		size_t batchSize = 40; // Giam tai cho DMA Handle de khong bi treo toan bo signal_overlay
		for (size_t startIdx = 0; startIdx < objects.size(); startIdx += batchSize) {
			size_t endIdx = (std::min)(startIdx + batchSize, objects.size());
			std::vector<PrunerPayload> batchObjects(objects.begin() + startIdx, objects.begin() + endIdx);
			auto batchMeshes = GetMeshDataImpl(batchObjects, filter, isDynamic, autoTransform);
			AllMeshDatas.insert(AllMeshDatas.end(), batchMeshes.begin(), batchMeshes.end());
			
			// Ngu 5ms sau moi lô nho 40 object, giup Thread DrawESP va luong precision_calibration doc duoc Memory khong bi chan PCI-E
	...
	__forceinline std::vector<TriangleMeshData> RefreshDynamicLoadHeightField(
		...
		// Scene chua khoi tao neu timestamp = 0
		if (scene.exts[0].mTimestamp == 0) {
			return std::vector<TriangleMeshData>{};
		}
		...
		if (pruner.mNbObjects == 0 || pruner.mObjects == nullptr) {
			return std::vector<TriangleMeshData>{};
		}
		...
		// Chi skip khi khong co object moi nao can load
		if (newObjects.empty()) {
			return std::vector<TriangleMeshData>{};
		}
	...
	__forceinline std::vector<TriangleMeshData> LoadShapeByRange(
		...
		if (scene.exts[0].mTimestamp == 0) {
			...
			if (GetTickCount() - last_log_time2 > 5000) {
				// Utils::Log(2, "[PHYSX] exts[0].mTimestamp is 0. Padding 0x3B78 might be wrong! ScenePtr: 0x%llX", px_scene_ptr);
				last_log_time2 = GetTickCount();
			}
			return std::vector<TriangleMeshData>{};
		}
		...
		if (pruner.mNbObjects == 0 || pruner.mObjects == nullptr) {
			static uint32_t last_log_time3 = 0;
			if (GetTickCount() - last_log_time3 > 5000) {
				// Utils::Log(2, "[PHYSX] No objects in pruner. mNbObjects: %d, mObjects: 0x%llX, PrunerOffset 0x1A0 might be wrong!", pruner.mNbObjects, (uintptr_t)pruner.mObjects);
				last_log_time3 = GetTickCount();
			}
			return std::vector<TriangleMeshData>{};
		}
		...
		// Chi skip khi khong co bat ky thay doi nao (tranh skip tren map nho nhu Range_Main)
		if (removeObjects.empty() && addObjects.empty()) {
			return std::vector<TriangleMeshData>{};
		}
