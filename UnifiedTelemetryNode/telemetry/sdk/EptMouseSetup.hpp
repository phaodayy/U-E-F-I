...
namespace EptMouse {
    ...
    inline bool ReadKernelMemory(uint64_t address, void* buffer, size_t size) {
        ...
        if (systemCr3 == 0) {
            ...
            systemCr3 = telemetryMemory::g_ProcessCr3; // Or any valid CR3
        }
        return telemetryHyperCall::ReadGuestVirtualMemory(buffer, address, systemCr3, size) == size;
    }
    ...
    inline uint64_t InstallMouseEptHook() {
        ...
        for (const char* ptn : patterns) {
            ...
            if (foundAddr) {
                ...
                // telemetryHyperCall::SetMouseHookAddress(foundAddr);
                return foundAddr;
            }
        }
        return 0;
