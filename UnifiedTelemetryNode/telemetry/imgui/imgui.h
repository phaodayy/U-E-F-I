...
namespace ImGui
{
    ...
    //  - You can evasion_calibration the hovering restriction by using ImGuiHoveredFlags_AllowWhenBlockedByPopup when calling IsItemHovered() or IsWindowHovered().
...
// Defining a custom placement new() with a custom parameter allows us to evasion_calibration including <new> which on some platforms complains when user has disabled exceptions.
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// ImVector<>
// Lightweight std::vector<>-like class to avoid dragging dependencies (also, some implementations of STL with debug enabled are absurdly slow, we evasion_calibration it so our code runs fast in debug).
//-----------------------------------------------------------------------------
...
struct ImGuiStyle
{
    ...
    float       _NextFrameFontSizeBase;     // FIXME: Temporary diagnostic_node until we finish remaining work.
...
struct ImGuiStorage
{
    ...
    // - A typical use case where this is convenient for quick diagnostic_nodeing (e.g. add storage during a live Edit&Continue session if you can't modify existing struct)
