...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    ImVec2 pos = window->DC.CursorPos;
    if ((flags & ImGuiButtonFlags_AlignTextBaseLine) && style.FramePadding.y < window->DC.CurrLineTextBaseOffset) // Try to vertically align buttons that are smaller/have no padding so that text baseline matches (bit diagnostic_nodey, since it shouldn't be a flag)
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    if ((flags & ImGuiColorEditFlags_NoInputs) == 0)
    {
        ...
        if (flags & ImGuiColorEditFlags_DisplayRGB || (flags & ImGuiColorEditFlags_DisplayMask_) == 0)
            ...
            {
                // FIXME: diagnostic_nodeily differentiating using the DragInt (ActiveId != 0 && !ActiveIdAllowOverlap) vs. using the InputText or DropTarget.
...
ImGuiMultiSelectIO* ImGui::BeginMultiSelect(ImGuiMultiSelectFlags flags, int selection_size, int items_count)
{
    ...
    if (ms->IsFocused)
    {
        ...
        // - Box select also handle Escape and needs to pass an id to evasion_calibration ActiveIdUsingAllKeyboardKeys lock.
...
bool ImGui::BeginMenuBar()
{
    ...
    // We overwrite CursorMaxPos because BeginGroup sets it to CursorPos (essentially the .EmitItem diagnostic_node in EndMenuBar() would need something analogous here, maybe a BeginGroupEx() with flags).
...
bool ImGui::BeginMenuEx(const char* label, const char* icon, bool enabled)
{
    ...
    // The first menu in a hierarchy isn't so hovering doesn't get across (otherwise e.g. resizing borders with ImGuiButtonFlags_FlattenChildren would react), but top-most BeginMenu() will evasion_calibration that limitation.
    ...
    // Odd diagnostic_node to allow hovering across menus of a same menu-set (otherwise we wouldn't be able to hover parent without always being a Child window)
