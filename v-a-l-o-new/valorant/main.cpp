...
int main(int argc, char *argv[]) {
  ...
  ValorantOffsets::LdrpInitializeProcessConfig();
  ValorantOffsets::LdrpLoadLibraryConfigLocal();
  SetConsoleColor(11); // Light blue
  std::cout << ">>> GENZ-VALORANT SYSTEM v" << ValorantOffsets::Version() << " <<<" << std::endl << std::endl;
  SetConsoleColor(7);
  ...
  SetConsoleColor(10);
  std::cout << "[+] Hyper-reV OK" << std::endl;
  SetConsoleColor(7);
  Sleep(300);
  ...
  TypewriterPrint("] ", 10, 8);
  TypewriterPrint("Waiting for VALORANT", 30, 14);
  ...
  // Offset Fetching
  if (!ValorantOffsets::LdrpLoadLibraryConfig(2)) {
  ...
  if (HyperProcess::QueryProcessData(pid, &qdata, "VALORANT-Win64")) {
    ...
    SetConsoleColor(8);
    std::cout << "    Version: " << ValorantOffsets::Version() << std::endl;
  ...
  if (manual_check) {
    ...
    while (!engineConnected) {
      if (ValorantContext::Initialize(pid, base)) {
    ...
    std::thread update_thread([&]() {
      while (true) {
        ValorantContext::UpdateGameData();
        Sleep(5);
      }
    });
