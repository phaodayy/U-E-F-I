...
std::string GetNameFast(int32_t index)
{
    ...
    uint64_t base = ValorantContext::GetBaseAddress();
    if (!base)
        return {};

    if (!g_StateInitialized) {
        uint64_t stateAddr = base + ValorantOffsets::FNameState() + ValorantOffsets::FNameKey();
        if (ValorantContext::ReadMemory(stateAddr, g_FNameState, sizeof(g_FNameState))) {
    ...
    uint64_t poolBase = base + ValorantOffsets::FNamePool();

    uint32_t block  = static_cast<uint32_t>(index >> 16);
    uint32_t offset = static_cast<uint32_t>(index & 0xFFFF);

    uint64_t blockPtr =
        ValorantContext::Read<uint64_t>(poolBase + 0x10 + 8ull * block);
    if (!blockPtr)
        return {};

    uint64_t entryPtr = blockPtr + 4ull * offset;

    uint32_t key = ValorantContext::Read<uint32_t>(entryPtr);
    uint16_t header = ValorantContext::Read<uint16_t>(entryPtr + 4);
    ...
    char buf[1024]{};
    if (!ValorantContext::ReadMemory(entryPtr + 6, buf, len))
        return {};
...
bool IsA(uint64_t actor, const std::string &class_name)
{
    ...
    uint64_t current = ValorantContext::Read<uint64_t>(actor + 0x10);
    while (current) {
        int32_t fnameIndex = ValorantContext::Read<int32_t>(current + 0x18);
        std::string name = GetNameFast(fnameIndex);
        if (name.find(class_name) != std::string::npos)
            return true;
        current = ValorantContext::Read<uint64_t>(current + 0x48);
    }
    return false;
}
