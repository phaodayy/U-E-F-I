...
void OverlayMenu::RenderFrame() {
  try {
    ...
    */
    ...
    // ========== ESP RENDERING (uses GUI:: namespace variables) ==========
    if (GUI::esp_toggle && G_PlayerController) {
      ImDrawList *draw = ImGui::GetBackgroundDrawList();

      static int debugFrameCount = 0;
      debugFrameCount++;

      uint64_t camera_manager = ValorantContext::Read<uint64_t>(
          G_PlayerController + ValorantOffsets::PlayerCameraManager());
      if (camera_manager) {
        struct DVector3 {
          double x, y, z;
        };
        ...
        FMinimalViewInfo viewInfo = ValorantContext::Read<FMinimalViewInfo>(
            camera_manager + ValorantOffsets::CameraCachePrivate() + 0x10);
        ...
        float fov_pixels = GUI::flick_fov * (ScreenWidth / 103.0f); // Approximate based on 103 standard FOV
        ...
        if (GUI::flick_toggle && !skip_input && (GetAsyncKeyState(GUI::flick_key) & 0x8000)) {
          ...
          for (const auto &player : CachedPlayers) {
            ...
            if (GUI::flick_bone == 0) target_bone_pos = player.Bone_Head;
            else if (GUI::flick_bone == 1) target_bone_pos = player.Bone_Neck;
            else if (GUI::flick_bone == 2) target_bone_pos = player.Bone_Chest;
          ...
          if (found) {
            ...
            if (abs(dx) > GUI::flick_deadzone || abs(dy) > GUI::flick_deadzone) {
                float smooth = GUI::flick_smooth;
                if (smooth < 1.0f) smooth = 1.0f;
                
                dx = (dx * GUI::flick_speed) / smooth;
                dy = (dy * GUI::flick_speed) / smooth;

                ValorantContext::MoveMouse((long)dx, (long)dy);
            }

            // Auto-fire check: only fire if VERY close to target (precision fire)
            if (GUI::flick_auto_fire && best_dist < 4.5f) {
        ...
        if (GUI::macro_toggle && !skip_input && (GetAsyncKeyState(VK_LBUTTON) & 0x8000)) {
            ...
            if (elapsed >= GUI::macro_delay) {
                ValorantContext::MoveMouse((long)GUI::macro_intensity_x, (long)GUI::macro_intensity_y);
                last_macro_time = now;
            }
        }
        ...
        for (const auto &player : CachedPlayers) {
          ...
          skel_col.Value.w *= (GUI::skel_alpha / 100.0f);
          ImColor box_col = line_color;
          box_col.Value.w *= (GUI::box_alpha / 100.0f);

          Vector3 box_root, box_head;
          if (player.MeshAddr && player.BoneArrayAddr) {
            box_root = ValorantContext::GetBoneWorldPos(0, player.BoneArrayAddr,
                                                        player.MeshAddr);
            box_head = ValorantContext::GetBoneWorldPos(8, player.BoneArrayAddr,
                                                        player.MeshAddr);
          } else {
          ...
          if (GUI::esp_box && GUI::esp_box_type <= 1) {
            if (w2s_success) {
              ...
              fill_col_rect.Value.w *= (GUI::fill_alpha / 100.0f);
              ...
              if (GUI::esp_box &&
                  (GUI::esp_box_type == 0 || GUI::esp_box_type == 1)) {
                ImU32 line_col = (ImU32)box_col;
                float th = 1.5f;
                if (GUI::esp_box_type == 0) {
          ...
          if (GUI::esp_head_circle && w2s_success) {
          ...
          // Health Bar - esp_health_type: 0=Top, 1=Bottom, 2=Left, 3=Right
          if (GUI::esp_health && w2s_success) {
          ...
          if (GUI::esp_box && GUI::esp_box_type == 2) {
            ...
            if (valid3d) {
              ...
              ImColor fill3d =
                  ImColor(fill_c[0], fill_c[1], fill_c[2], fill_c[3]);
              fill3d.Value.w *= (GUI::fill_alpha / 100.0f);
              if (GUI::esp_fillbox) {
                // Tüm 6 yüzeyi doldur
                draw->AddQuadFilled(
          ...
          if (GUI::esp_skeleton && player.BoneCount > 0 && player.MeshAddr &&
              player.BoneArrayAddr) {
            uint64_t ba = player.BoneArrayAddr;
            uint64_t m = player.MeshAddr;
            int bc = player.BoneCount;

            Vector3 bHead = ValorantContext::GetBoneWorldPos(8, ba, m);
            Vector3 bChest = ValorantContext::GetBoneWorldPos(6, ba, m);
            Vector3 bPelvis = ValorantContext::GetBoneWorldPos(3, ba, m);
            Vector3 bNeck, bLS, bLE, bLH, bRS, bRE, bRH, bLT, bLK, bLF, bRT,
                bRK, bRF;

            if (bc == 103) { // BOT
              bNeck = ValorantContext::GetBoneWorldPos(9, ba, m);
              bLS = ValorantContext::GetBoneWorldPos(33, ba, m);
              bLE = ValorantContext::GetBoneWorldPos(30, ba, m);
              bLH = ValorantContext::GetBoneWorldPos(32, ba, m);
              bRS = ValorantContext::GetBoneWorldPos(58, ba, m);
              bRE = ValorantContext::GetBoneWorldPos(55, ba, m);
              bRH = ValorantContext::GetBoneWorldPos(57, ba, m);
              bLT = ValorantContext::GetBoneWorldPos(63, ba, m);
              bLK = ValorantContext::GetBoneWorldPos(65, ba, m);
              bLF = ValorantContext::GetBoneWorldPos(69, ba, m);
              bRT = ValorantContext::GetBoneWorldPos(77, ba, m);
              bRK = ValorantContext::GetBoneWorldPos(79, ba, m);
              bRF = ValorantContext::GetBoneWorldPos(83, ba, m);
            } else if (bc == 104) { // MALE
              bNeck = ValorantContext::GetBoneWorldPos(21, ba, m);
              bLS = ValorantContext::GetBoneWorldPos(23, ba, m);
              bLE = ValorantContext::GetBoneWorldPos(24, ba, m);
              bLH = ValorantContext::GetBoneWorldPos(25, ba, m);
              bRS = ValorantContext::GetBoneWorldPos(49, ba, m);
              bRE = ValorantContext::GetBoneWorldPos(50, ba, m);
              bRH = ValorantContext::GetBoneWorldPos(51, ba, m);
              bLT = ValorantContext::GetBoneWorldPos(77, ba, m);
              bLK = ValorantContext::GetBoneWorldPos(78, ba, m);
              bLF = ValorantContext::GetBoneWorldPos(80, ba, m);
              bRT = ValorantContext::GetBoneWorldPos(84, ba, m);
              bRK = ValorantContext::GetBoneWorldPos(85, ba, m);
              bRF = ValorantContext::GetBoneWorldPos(87, ba, m);
              ...
              bRE = ValorantContext::GetBoneWorldPos(50, ba, m);
              bRH = ValorantContext::GetBoneWorldPos(51, ba, m);
              bLT = ValorantContext::GetBoneWorldPos(75, ba, m);
              bLK = ValorantContext::GetBoneWorldPos(76, ba, m);
              bLF = ValorantContext::GetBoneWorldPos(78, ba, m);
              bRT = ValorantContext::GetBoneWorldPos(82, ba, m);
              bRK = ValorantContext::GetBoneWorldPos(83, ba, m);
              bRF = ValorantContext::GetBoneWorldPos(85, ba, m);
            }

            auto DrawBoneLine = [&](Vector3 from, Vector3 to) {
              Vector2 s1, s2;
              if (WorldToScreen(from, s1) && WorldToScreen(to, s2)) {
                float thickness = GUI::skel_thickness / 10.0f;
          ...
          if (GUI::esp_snapline) {
            ...
            if (WorldToScreen(box_root, snap_target)) {
              ...
              ImVec2 from;
              if (GUI::snapline_type == 0) // Bottom
                from = ImVec2(ScreenWidth / 2, ScreenHeight);
              else if (GUI::snapline_type == 1) // Top
          ...
          if (GUI::esp_weapon && !player.WeaponName.empty()) {
            Vector2 ft_scr;
            if (WorldToScreen(box_root, ft_scr)) {
              float yOff = GUI::esp_distance ? 22.0f : 5.0f;
