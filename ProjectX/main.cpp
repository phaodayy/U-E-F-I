...
bool CheckBlacklistedProcesses() {
    const wchar_t* blacklisted[] = { L"ida64.exe", L"x64dbg.exe", L"internalengine-x86_64.exe", L"Fiddler.exe", L"wireshark.exe", L"HTTPDebuggerSvc.exe", L"ida.exe", L"x32dbg.exe" };
    HANDLE snap = CreateToolhelp32Snapshot(TH32CS_SNAPPROCESS, 0);
    PROCESSENTRY32W pe = {sizeof(pe)};
    if (Process32FirstW(snap, &pe)) {
        do {
...
bool SecurityCheck() {
    ...
    if (FindWindowA(skCrypt("Window"), skCrypt("Internal Engine"))) return false;
...
int main(int argc, char *argv[]) {
  ...
  ProjectXOffsets::LdrpInitializeProcessConfig();
  ProjectXOffsets::LdrpLoadLibraryConfigLocal();
  SetConsoleColor(11); // Light blue
  std::cout << ">>> GENZ-ProjectX SYSTEM v" << ProjectXOffsets::Version() << " <<<" << std::endl << std::endl;
  SetConsoleColor(7);
  ...
  TypewriterPrint("] ", 10, 8);
  TypewriterPrint("Waiting for ProjectX", 30, 14);

  DWORD pid = 0;
  int dots = 0;
  while (!pid) {
    pid = GetProcessIdByName(L"ProjectX-Win64-Shipping.exe");
    if (!pid) {
  ...
  // Offset Fetching
  if (!ProjectXOffsets::LdrpLoadLibraryConfig(2)) {
  ...
  if (Driver::QueryProcess(pid, &qdata)) {
    ...
    SetConsoleColor(8);
    std::cout << "    Version: " << ProjectXOffsets::Version() << std::endl;
    std::cout << "    Base: 0x" << std::hex << (uint64_t)qdata.base_address << std::dec << std::endl;
    std::cout << "    CR3:  0x" << std::hex << qdata.cr3 << std::dec << std::endl;
    std::cout << "    PEB:  0x" << std::hex << (uint64_t)qdata.peb << std::dec << std::endl;
    SetConsoleColor(7);
  } else {
    SetConsoleColor(12);
  ...
  if (mz == 0x5A4D) {
    ...
    while (!engineConnected) {
      if (ProjectXContext::Initialize(pid, base)) {
    ...
    std::thread update_thread([&]() {
      while (true) {
        ProjectXContext::UpdateGameData();
        Sleep(5);
      }
    });
