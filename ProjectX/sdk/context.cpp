...
Vector3 GetBoneWorldPos(int boneIndex, uint64_t boneArray, uint64_t meshPtr) {
  DFTransform compToWorld =
      Read<DFTransform>(meshPtr + ProjectXOffsets::ComponentToWorld());
...
bool Initialize(uint32_t process_id, uint64_t base_address) {
  ...
  uint64_t uworldPtr1 =
      Read<uint64_t>(g_BaseAddress + ProjectXOffsets::GWorld());
  G_UWorld = Read<uint64_t>(uworldPtr1);

  if (G_UWorld && G_UWorld > 0x10000) {
    G_GameInstance = Read<uint64_t>(G_UWorld + ProjectXOffsets::GameInstance());
    G_PersistentLevel =
        Read<uint64_t>(G_UWorld + ProjectXOffsets::PersistentLevel());
...
void UpdateGameData() {
  ...
  uint64_t uworldPtr1 = Read<uint64_t>(g_BaseAddress + ProjectXOffsets::GWorld());
  ...
  */
  ...
  if (!G_LocalPlayer || !G_PlayerController || !G_LocalPawn) {
    uint64_t localPlayersArray =
        Read<uint64_t>(G_GameInstance + ProjectXOffsets::LocalPlayers());
    if (localPlayersArray) {
      G_LocalPlayer = Read<uint64_t>(localPlayersArray);
      if (G_LocalPlayer) {
        G_PlayerController =
            Read<uint64_t>(G_LocalPlayer + ProjectXOffsets::PlayerController());
        if (G_PlayerController) {
          G_LocalPawn = Read<uint64_t>(G_PlayerController +
                                       ProjectXOffsets::AcknowledgedPawn());
        }
      }
    }
    /*
    ...
    */
    ...
    uint64_t checkPawn =
        Read<uint64_t>(G_PlayerController + ProjectXOffsets::AcknowledgedPawn());
  ...
  if (G_PlayerController) {
    uint64_t camMgr = Read<uint64_t>(G_PlayerController +
                                     ProjectXOffsets::PlayerCameraManager());
    if (camMgr) {
      struct DV3 {
        double x, y, z;
      };
      ...
      FMinViewInfo vi = Read<FMinViewInfo>(
          camMgr + ProjectXOffsets::CameraCachePrivate() + 0x10);
  ...
  uint64_t localMesh = Read<uint64_t>(G_LocalPawn + ProjectXOffsets::Mesh());
  Vector3 localPos = {0, 0, 0};
  if (localMesh) {
    uint64_t localBoneArr =
        Read<uint64_t>(localMesh + ProjectXOffsets::BoneArray());
    if (!localBoneArr)
      localBoneArr =
          Read<uint64_t>(localMesh + ProjectXOffsets::BoneArrayCache());
    if (localBoneArr)
  ...
  uint64_t actorArray =
      Read<uint64_t>(G_PersistentLevel + ProjectXOffsets::ActorArray());
  int actorCount = Read<int>(G_PersistentLevel + ProjectXOffsets::ActorCount());
  ...
  */
  ...
  for (int i = 0; i < actorCount; i++) {
    ...
    uint64_t mesh = Read<uint64_t>(actor + ProjectXOffsets::Mesh());
    if (!mesh) {
      actorSkips_mesh++;
      continue;
    }
    ...
    uint64_t damageHandler =
        Read<uint64_t>(actor + ProjectXOffsets::DamageHandler());
    if (!damageHandler) {
      actorSkips_damage++;
      continue;
    }
    ...
    if (GUI::esp_team_check) {
      bool wasAlly = Read<bool>(actor + ProjectXOffsets::WasAlly());
      if (wasAlly) {
        actorSkips_team++;
        continue;
      }
    ...
    float health = Read<float>(damageHandler + ProjectXOffsets::Health());
    if (health <= 0.0f || health > 200.0f) {
      actorSkips_health++;
      continue;
    }

    uint64_t boneArray = Read<uint64_t>(mesh + ProjectXOffsets::BoneArray());
    if (!boneArray)
      boneArray = Read<uint64_t>(mesh + ProjectXOffsets::BoneArrayCache());
    if (!boneArray) {
      actorSkips_bones++;
      continue;
    }
    ...
    int boneCount = Read<int>(mesh + ProjectXOffsets::BoneCount());
    ...
    player.bIsAlive = Read<bool>(damageHandler + ProjectXOffsets::bAlive());
    player.bIsBot = (player.Name == "Bot");

    float lastSubmit = Read<float>(mesh + ProjectXOffsets::LastSubmitTime());
    float lastRender =
        Read<float>(mesh + ProjectXOffsets::LastSubmitTimeOnScreen());
    ...
    uint64_t inventory = Read<uint64_t>(actor + ProjectXOffsets::Inventory());
    if (inventory) {
      uint64_t equippable =
          Read<uint64_t>(inventory + ProjectXOffsets::CurrentEquippable());
      if (equippable) {
