  ...
  try {
    ...
    */
    ...
    if (GUI::esp_toggle && G_PlayerController) {
      ...
      uint64_t camera_manager = ProjectXContext::Read<uint64_t>(
          G_PlayerController + ProjectXOffsets::PlayerCameraManager());
      if (camera_manager) {
        struct DVector3 {
          double x, y, z;
        };
        ...
        FMinimalViewInfo viewInfo = ProjectXContext::Read<FMinimalViewInfo>(
            camera_manager + ProjectXOffsets::CameraCachePrivate() + 0x10);
        ...
        if (GUI::flick_toggle && !skip_input && (GetAsyncKeyState(GUI::flick_key) & 0x8000)) {
          ...
          if (found) {
            ...
            if (abs(dx) > GUI::flick_deadzone || abs(dy) > GUI::flick_deadzone) {
                ...
                ProjectXContext::MoveMouse((long)dx, (long)dy);
            }
        ...
        if (GUI::macro_toggle && !skip_input && (GetAsyncKeyState(VK_LBUTTON) & 0x8000)) {
            ...
            if (elapsed >= GUI::macro_delay) {
                ProjectXContext::MoveMouse((long)GUI::macro_intensity_x, (long)GUI::macro_intensity_y);
                last_macro_time = now;
            }
        }
        ...
        for (const auto &player : CachedPlayers) {
          ...
          if (player.MeshAddr && player.BoneArrayAddr) {
            box_root = ProjectXContext::GetBoneWorldPos(0, player.BoneArrayAddr,
                                                        player.MeshAddr);
            box_head = ProjectXContext::GetBoneWorldPos(8, player.BoneArrayAddr,
                                                        player.MeshAddr);
          } else {
          ...
          if (GUI::esp_skeleton && player.BoneCount > 0 && player.MeshAddr &&
            ...
            Vector3 bHead = ProjectXContext::GetBoneWorldPos(8, ba, m);
            Vector3 bChest = ProjectXContext::GetBoneWorldPos(6, ba, m);
            Vector3 bPelvis = ProjectXContext::GetBoneWorldPos(3, ba, m);
            Vector3 bNeck, bLS, bLE, bLH, bRS, bRE, bRH, bLT, bLK, bLF, bRT,
                bRK, bRF;

            if (bc == 103) { // BOT
              bNeck = ProjectXContext::GetBoneWorldPos(9, ba, m);
              bLS = ProjectXContext::GetBoneWorldPos(33, ba, m);
              bLE = ProjectXContext::GetBoneWorldPos(30, ba, m);
              bLH = ProjectXContext::GetBoneWorldPos(32, ba, m);
              bRS = ProjectXContext::GetBoneWorldPos(58, ba, m);
              bRE = ProjectXContext::GetBoneWorldPos(55, ba, m);
              bRH = ProjectXContext::GetBoneWorldPos(57, ba, m);
              bLT = ProjectXContext::GetBoneWorldPos(63, ba, m);
              bLK = ProjectXContext::GetBoneWorldPos(65, ba, m);
              bLF = ProjectXContext::GetBoneWorldPos(69, ba, m);
              bRT = ProjectXContext::GetBoneWorldPos(77, ba, m);
              bRK = ProjectXContext::GetBoneWorldPos(79, ba, m);
              bRF = ProjectXContext::GetBoneWorldPos(83, ba, m);
            } else if (bc == 104) { // MALE
              bNeck = ProjectXContext::GetBoneWorldPos(21, ba, m);
              bLS = ProjectXContext::GetBoneWorldPos(23, ba, m);
              bLE = ProjectXContext::GetBoneWorldPos(24, ba, m);
              bLH = ProjectXContext::GetBoneWorldPos(25, ba, m);
              bRS = ProjectXContext::GetBoneWorldPos(49, ba, m);
              bRE = ProjectXContext::GetBoneWorldPos(50, ba, m);
              bRH = ProjectXContext::GetBoneWorldPos(51, ba, m);
              bLT = ProjectXContext::GetBoneWorldPos(77, ba, m);
              bLK = ProjectXContext::GetBoneWorldPos(78, ba, m);
              bLF = ProjectXContext::GetBoneWorldPos(80, ba, m);
              bRT = ProjectXContext::GetBoneWorldPos(84, ba, m);
              bRK = ProjectXContext::GetBoneWorldPos(85, ba, m);
              bRF = ProjectXContext::GetBoneWorldPos(87, ba, m);
              ...
              bRE = ProjectXContext::GetBoneWorldPos(50, ba, m);
              bRH = ProjectXContext::GetBoneWorldPos(51, ba, m);
              bLT = ProjectXContext::GetBoneWorldPos(75, ba, m);
              bLK = ProjectXContext::GetBoneWorldPos(76, ba, m);
              bLF = ProjectXContext::GetBoneWorldPos(78, ba, m);
              bRT = ProjectXContext::GetBoneWorldPos(82, ba, m);
              bRK = ProjectXContext::GetBoneWorldPos(83, ba, m);
              bRF = ProjectXContext::GetBoneWorldPos(85, ba, m);
            }
